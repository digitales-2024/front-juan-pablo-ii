...
import { updateRolesSchema, UpdateRolesSchema } from "@/schemas/users/rolesSchema";
import { Role } from "@/types";
import { zodResolver } from "@hookform/resolvers/zod";
import { ChevronDown, ChevronUp, Circle, RefreshCcw, Edit3 } from "lucide-react";
...
interface UpdateRoleSheetProps extends React.ComponentPropsWithRef<typeof Sheet> {
  role: Role;
}
...
export function UpdateRoleSheet({ role, ...props }: UpdateRoleSheetProps) {
  const { dataRolPermissions, onUpdateRole, isSuccessUpdateRole, isLoadingUpdateRole } = useRol();
  ...
  const idsModulesPermissions: string[] =
    role.rolPermissions &&
    role.rolPermissions.flatMap((rolePerm) =>
      rolePerm.permissions.map((permission) => permission.idModulePermission).filter((id): id is string => id !== undefined)
    );
  ...
  return (
    <>
      <Button variant="ghost" onClick={() => setIsSheetOpen(true)} size="icon" aria-label="Abrir editor de rol">
        <Edit3 className="h-5 w-5" />
      </Button>

      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen} {...props}>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 p-2">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {dataRolPermissions?.map(({ module, permissions }) => (
                    ...
                    >
                      <div className="flex cursor-pointer items-center justify-between p-4" onClick={() => toggleModule(module.name)}>
                        <div className="flex items-center space-x-2">
                          <span className="truncate font-medium">{module.name}</span>
                        </div>
                        {expandedModules.includes(module.name) ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                      </div>
                          ...
                          {permissions.map((permission) => (
                            <div key={permission.id} className="flex items-center justify-between space-x-2">
                              <Label
                                ...
                                className={cn("inline-flex items-center gap-2 text-sm capitalize", {
                                  "text-emerald-500": selectedPermissions.includes(`${permission.idModulePermission}`),
                                })}
                              >
                                <Circle
                                  size={10}
                                  className={cn("fill-slate-300 stroke-none", {
                                    "fill-emerald-500": selectedPermissions.includes(`${permission.idModulePermission}`),
                                  })}
                                />
                ...
                <SheetFooter className="flexgap-2 pt-2 sm:space-x-0">
                  <div className="flex flex-row-reverse flex-wrap gap-2">
                    <Button disabled={isLoadingUpdateRole}>
                      {isLoadingUpdateRole && <RefreshCcw className="mr-2 size-4 animate-spin" aria-hidden="true" />}
                      Actualizar
                    </Button>
                    <SheetClose asChild>
